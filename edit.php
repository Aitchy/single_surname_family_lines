...
if (filter_input(INPUT_POST, 'mutch')  and (strlen(filter_input(INPUT_POST, 'mname'))  > 1)) {
// split name string
//$tempname=$_POST[mname];
//$tempname1=$_POST[mname];
  $name = split(" ", filter_input(INPUT_POST, 'mname'), 3);
  $name1=($name[0]);
  $name2=($name[1]);
  $name3=($name[2]);
...
if ($result_o->num_rows > 0){
  ...
  echo(" Mutch");
  echo(" returned ".$result_o->num_rows." hits in the supplementary database, select below...</font>");
  echo("</p>");
  echo("<ul>");
  while ($row_sup = $result_o->fetch_assoc()) {
// Get the correct Mutch link for child
    $sel = "select * from Unions where id = ".$row_sup["child_of"]." order by dom_year";
...
if (filter_input(INPUT_POST, 'spouse')  and (strlen(filter_input(INPUT_POST, 'sname'))  > 1)) {
// split name string
	$sname = split(" ", filter_input(INPUT_POST, 'sname'), 3);
	$sname1=($sname[0]);
	$sname2=($sname[1]);
	$sname3=($sname[2]);
